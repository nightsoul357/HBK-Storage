namespace CG.Web.MegaApiClient
{
    ...
    internal abstract class MegaAesCtrStream : Stream
    {
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            ...
            var closest = _chunksPositionsCache.Where(x => x <= tempPosition).Max();
            var pt = _stream.Seek(closest, SeekOrigin.Begin);

            long times = closest / 16;
            ...
            _position = closest;
            byte[] buffer = new byte[tempPosition - closest];
