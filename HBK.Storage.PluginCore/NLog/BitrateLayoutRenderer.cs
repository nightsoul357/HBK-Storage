...
{
    [LayoutRenderer("plugin_ffpmeg_bitrate")]
    public class BitrateLayoutRenderer : PluginLayoutRendererBase
    {
        protected override void Append(StringBuilder builder, LogEventInfo logEvent)
        {
            if (base.TryFetchPluginLogEvent(logEvent, out PluginLogEvent pluginLogEvent))
            {
                ...
                if (obj.ContainsKey("Bitrate"))
                {
                    builder.Append(obj["Bitrate"].Value<double?>());
                }
            }
        }
    }
